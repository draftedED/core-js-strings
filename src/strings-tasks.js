...
function removeFirstOccurrences(str, value) {
  return !str.includes(value)
    ? str
    : `${str.substring(0, str.indexOf(value))}${str.substring(
...
function removeLastOccurrences(str, value) {
  return !str.includes(value)
    ? str
    : `${str.substring(0, str.lastIndexOf(value))}${str.substring(
...
function sumOfCodes(str) {
  return str === undefined || str === null
    ...
    : str
        .split('')
        .map((item) => item.replace(item, item.charCodeAt()))
        .reduce((accum, item) => accum + parseFloat(item), 0);
}
...
function countVowels(str) {
  ...
  return str
    .split('')
    .reduce((accum, item) => (vowels.includes(item) ? accum + 1 : accum), 0);
}
...
function isPalindrome(str) {
  return str
    ...
    .every(
      (item, index) =>
        item === str.replaceAll(/\W/g, '').toLowerCase().split('')[index]
    );
}
...
function findLongestWord(sentence) {
  ...
  sentence.split(' ').forEach((item) => {
    result = item.length > result.length ? (result = item) : result;
  });
  return result;
}
...
function encodeToRot13(str) {
  ...
  return str
    .split('')
    .map((element) => ROT13[alphabet.indexOf(element)])
    .join('');
}
